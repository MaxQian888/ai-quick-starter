...
class BuildAgentsTeamTests(unittest.TestCase):
    ...
    def make_claude_home(self) -> Path:
        root = Path(tempfile.mkdtemp(prefix="claude-home-"))
    ...
    def test_claude_code_target_emits_markdown_files_with_frontmatter(self) -> None:
        input_path = self.make_input("Build docs, frontend, backend, and a testing pass for a new web app.")
        output_dir = self.make_output_dir()
        ...
        self.assertFalse(payload["toml_files"])
        self.assertTrue(payload["claude_code_files"])
        self.assertIn("default.md", payload["claude_code_files"])
        self.assertIn("worker.md", payload["claude_code_files"])
        self.assertIn("explorer.md", payload["claude_code_files"])

        explorer = payload["claude_code_files"]["explorer.md"]["content"]
        # Frontmatter must lead the file and carry required + optional fields
        # in the format Claude Code expects (see docs.claude.com/.../sub-agents).
        self.assertTrue(explorer.startswith("---\n"))
        self.assertIn("name: explorer", explorer)
        self.assertIn("description:", explorer)
        # Read-heavy roles get the read-only tool allowlist + haiku model.
        self.assertIn("tools: Read, Grep, Glob, WebFetch, WebSearch", explorer)
        self.assertIn("model: haiku", explorer)

        worker = payload["claude_code_files"]["worker.md"]["content"]
        # Workspace-write roles default to sonnet so write tools have headroom.
        self.assertIn("model: sonnet", worker)
        self.assertIn("tools: Read, Edit, Write, Bash", worker)

        # The skill must produce a paste-in team brief because Claude Code
        # team configs are runtime-managed and not pre-authorable.
        self.assertTrue(payload["team_brief"])
        brief_path = self.read_output_path(result.stdout, "TEAM_BRIEF")
        brief_text = brief_path.read_text(encoding="utf-8")
        self.assertIn("CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS", brief_text)
        self.assertIn("Spawn These Teammates", brief_text)
        self.assertIn("Coordination Rules", brief_text)

    def test_target_both_emits_codex_and_claude_code_artifacts(self) -> None:
        input_path = self.make_input("Build docs, frontend, backend, and verification.")
        output_dir = self.make_output_dir()
        ...
        self.assertEqual(result.returncode, 0, msg=result.stderr or result.stdout)
        payload = self.load_json_from_stdout(result.stdout)
        self.assertTrue(payload["toml_files"])
        self.assertTrue(payload["claude_code_files"])
        self.assertEqual(payload["target"], "both")

        # Markdown report should embed both per-target sections so a reviewer
        # can compare the renderings without opening more files.
        markdown_path = self.read_output_path(result.stdout, "MARKDOWN_OUT")
        markdown = markdown_path.read_text(encoding="utf-8")
        self.assertIn("## TOML Drafts", markdown)
        self.assertIn("## Claude Code Subagent Files", markdown)
        self.assertIn("## Claude Code Team Brief", markdown)

    def test_install_with_claude_code_target_writes_to_claude_home(self) -> None:
        input_path = self.make_input("Build docs, frontend, backend, and tests.")
        output_dir = self.make_output_dir()
        claude_home = self.make_claude_home()
        ...
        self.assertTrue((agents_dir / "default.md").exists())
        self.assertTrue((agents_dir / "worker.md").exists())
        self.assertTrue((agents_dir / "explorer.md").exists())
        manifest = payload["install_manifest"]
        # Single target → manifest stays a dict for backwards compatibility.
        self.assertEqual(manifest["target"], "claude-code")
        self.assertTrue(Path(manifest["path"]).exists())

    def test_install_with_target_both_records_manifest_per_target(self) -> None:
        input_path = self.make_input("Build docs, frontend, backend, and tests.")
        output_dir = self.make_output_dir()
        codex_home = self.make_codex_home()
        claude_home = self.make_claude_home()
        ...
        self.assertEqual(result.returncode, 0, msg=result.stderr or result.stdout)
        payload = self.load_json_from_stdout(result.stdout)
        self.assertTrue((codex_home / "agents" / "default.toml").exists())
        self.assertTrue((claude_home / "agents" / "default.md").exists())
        # With both targets we expose a list of manifests so callers can route
        # uninstall per target without re-deriving paths.
        manifest = payload["install_manifest"]
        self.assertIsInstance(manifest, list)
        targets = {entry["target"] for entry in manifest}
        self.assertEqual(targets, {"codex", "claude-code"})

        # Uninstall both in one call.
        uninstall_result = self.run_cli(
            "--uninstall",
            "--project-name",
            "dual-install",
        ...
        self.assertEqual(uninstall_result.returncode, 0, msg=uninstall_result.stderr or uninstall_result.stdout)
        self.assertFalse((codex_home / "agents" / "default.toml").exists())
        self.assertFalse((claude_home / "agents" / "default.md").exists())
