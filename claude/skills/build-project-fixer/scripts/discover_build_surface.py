...
COMMAND_KEYWORDS = {
    "test": ("pytest", "vitest", "jest", "unittest", "cargo test", "pnpm test", "npm test", "yarn test", "go test", "playwright test"),
    "lint": ("eslint", "ruff", "flake8", "golangci-lint", "pnpm lint", "npm run lint", "yarn lint", "biome check", "biome lint"),
    "typecheck": ("mypy", "pyright", "tsc --noemit", "typecheck", "cargo check"),
    "build": (" build", "vite build", "next build", "cargo build", "python -m build", "tsc -b", "tsc --build", "go build"),
    "verify": ("check", "verify", "ci", "validate", "go vet"),
}
...
def infer_node_package_manager(root: Path, package_data: dict[str, object]) -> list[str]:
    ...
    if not detected and (root / "package.json").exists():
        detected.append("npm")
    return detected
...
def discover_pyproject(root: Path, state: DiscoveryState) -> None:
    ...
    for manager in install_candidates:
        state.add_package_manager(manager)
    ...
    elif isinstance(data, dict) and ("project" in data or "build-system" in data):
        state.add_command(
            category="install",
            command="python -m pip install -e .",
            source="pyproject.toml",
            evidence="pyproject.toml declares an installable project but no uv/poetry lockfile is present.",
            priority=40,
        )
...
def discover_gomod(root: Path, state: DiscoveryState) -> None:
    go_mod = root / "go.mod"
    if not go_mod.exists():
        return
    state.add_manifest(go_mod, "go.mod")
    state.add_package_manager("go")
    state.add_command(
        category="install",
        command="go mod download",
        source="go.mod",
        evidence="Go module manifest detected.",
        priority=55,
    )
    state.add_command(
        category="build",
...
def _iter_workflow_run_commands(text: str) -> list[str]:
    """Yield individual shell commands extracted from GitHub Actions ``run:`` steps.

    Handles three common shapes:
      1. inline:    ``- run: pnpm test``
      2. folded:    ``run: >`` followed by indented continuation lines
      3. literal:   ``run: |`` followed by indented continuation lines (multi-cmd)
    """
    commands: list[str] = []
    ...
    while i < len(lines):
        ...
        if stripped.startswith("- run:"):
            payload = stripped[len("- run:"):].strip()
        ...
        if payload in ("|", ">", "|-", ">-", "|+", ">+"):
            ...
            while j < len(lines):
                ...
                if cont_indent < block_indent:
                    break
                collected.append(cont[block_indent:].rstrip())
                j += 1
            block = "\n".join(collected)
            if payload.startswith("|"):
                ...
                folded = " ".join(part for part in block.split() if part)
                if folded:
                    commands.append(folded)
            i = j
            continue
...
def extract_ci_commands(root: Path, state: DiscoveryState) -> None:
    ...
    for workflow_path in sorted(workflow_dir.glob("*.y*ml")):
        ...
        for command in _iter_workflow_run_commands(text):
