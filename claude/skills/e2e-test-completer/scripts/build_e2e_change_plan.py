...
def walk_pruned(root: Path) -> Iterator[Path]:
    """Yield every file under ``root`` while pruning skip directories before descent.

    ``Path.rglob`` would happily descend into ``node_modules`` and friends and
    only filter the result list afterwards, which is pathologically slow on large
    monorepos. ``os.walk`` lets us mutate ``dirnames`` in place to prune the
    walk before the OS lists thousands of irrelevant entries.
    """
...
def find_repo_package_dirs(root: Path) -> list[str]:
    ...
    for file_path in walk_pruned(root):
        if file_path.name != "package.json":
            continue
        rel_dir = normalize_rel(file_path.parent.relative_to(root))
...
def find_config_paths(root: Path, filenames: tuple[str, ...]) -> list[str]:
    ...
    for file_path in walk_pruned(root):
        if file_path.name in targets:
            paths.append(normalize_rel(file_path.relative_to(root)))
    return sorted(paths)
...
def find_script_name(framework: str, package_data: dict[str, object]) -> str:
    ...
    framework_marker = framework if framework in {"playwright", "cypress"} else ""

    if framework_marker:
        for key, value in scripts.items():
            if framework_marker in str(value).lower():
                return str(key)

    for key, value in scripts.items():
        key_text = str(key).lower()
        if "e2e" in key_text and (
            not framework_marker
            or framework_marker in str(value).lower()
            or not _value_targets_other_framework(framework_marker, str(value).lower())
        ):
            return str(key)
    return ""


def _value_targets_other_framework(framework_marker: str, value_text: str) -> bool:
    if framework_marker == "playwright":
        return "cypress" in value_text
    if framework_marker == "cypress":
        return "playwright" in value_text
    return False
