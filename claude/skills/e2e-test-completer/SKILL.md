...
Bring E2E tests back in sync with the code before editing assertions. The bundled helper script discovers the current runner, ranks affected specs from changed files, surfaces uncovered flows, and builds a dry-run command plan — so the first real test run is the narrowest one that actually proves the change.

## How to invoke the helper

The helper is a single Python file with **no third-party dependencies** (standard library only, Python 3.9+). Pick whichever invocation fits the host environment:

```bash
# Plain interpreter — works everywhere Python 3.9+ is installed.
python3 scripts/build_e2e_change_plan.py --project-root . --mode discover --json

# Equivalent via uv (use this when the repo standardises on uv).
uv run --python 3.11 scripts/build_e2e_change_plan.py --project-root . --mode discover --json
```

Examples below use `python3` for portability. Substitute `uv run --python 3.11` when the project requires it.
...
1. Detect the E2E runner:
   ...
   - Check `package.json` scripts for `test:e2e`, `e2e`, or `playwright`/`cypress` commands.
...
The helper does most of this for you — read the JSON it returns instead of re-deriving it by hand.
...
2. **Build a change-driven E2E plan.**
   - When the user gives explicit changed paths:
     ```bash
     python3 scripts/build_e2e_change_plan.py --project-root . --mode plan \
       --changed-file src/features/auth/login-form.tsx --json
     ```
   ...
   - For branch-relative change detection, use:
     ```bash
     python3 scripts/build_e2e_change_plan.py --project-root . --mode plan --git-base origin/main --json
     ```
...
4. **Simulate the first verification command before widening.**
   ```bash
   python3 scripts/build_e2e_change_plan.py --project-root . --mode simulate \
     --changed-file src/features/auth/login-form.tsx --json
   ```
...
- Read `references/linked-skills.md` when E2E work spills into build failures, missing unit tests, or the user is ready to commit — it points at the right sibling skill to hand off to.
