...
description: |
  Use this skill whenever the user wants to plan or add project- or app-level internationalization (i18n).
  Make sure to use this skill whenever the user mentions i18n, internationalization, localization, l10n,
  translation, multi-language support, locale routing, or adding a new language to an app. Also trigger
  for requests like "translate my app", "add Spanish/Chinese/Japanese support", "set up react-i18next",
  "configure next-intl", "add vue-i18n", "wire up locale switching", "make this multilingual", or
  "how do I localize this app". Covers Next.js, React SPA, Vue, Python, CLI, and mixed-framework
  repositories. Inspect the existing stack before recommending anything so you never introduce a second
  conflicting i18n system. For a single component or bounded directory, hand off to
  guarded-component-i18n-fix instead.
---
...
2. Run the planner before picking a framework. The planner uses only the Python standard library, so any Python 3.10+ works — no `uv`, virtualenv, or extra packages are required.

   From inside this skill's folder:

   ```bash
   python3 scripts/build_project_i18n_plan.py --root <path-to-target-repo>
   ```

   From anywhere (substitute the absolute path to where this skill is installed, e.g. `~/.claude/skills/guarded-project-i18n-support`):

   ```bash
   python3 /absolute/path/to/guarded-project-i18n-support/scripts/build_project_i18n_plan.py --root <path-to-target-repo>
   ```

   On Windows, use `python` instead of `python3` if `python3` is not on PATH. The planner prints `JSON_OUT=...` and `MARKDOWN_OUT=...` lines pointing at the generated reports.
...
```bash
python3 scripts/build_project_i18n_plan.py --root .
