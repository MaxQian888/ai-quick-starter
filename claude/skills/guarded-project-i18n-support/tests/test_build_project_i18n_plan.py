...
SCRIPT = SKILL_ROOT / "scripts" / "build_project_i18n_plan.py"


def _resolve_python() -> str:
    override = os.environ.get("GUARDED_I18N_PYTHON")
    if override:
        return override
    workspace_python = (
        SKILL_ROOT.parent
...
class BuildProjectI18nPlanTests(unittest.TestCase):
    def make_repo(self, files: dict[str, str]) -> Path:
        temp_root = SKILL_ROOT / ".tmp-tests"
    ...
    def test_detects_vanilla_i18next_without_react_bindings(self) -> None:
        repo_root = self.make_repo(
            {
                "package.json": json.dumps(
                    {
                ...
                "src/app.js": "import i18n from './i18n.js';\nconsole.log(i18n.t('hello'));\n",
            }
        )
        ...
        self.assertEqual(result.returncode, 0, msg=result.stderr or result.stdout)
        payload = self.load_payload(result.stdout)
        self.assertEqual(payload["selected_system"]["name"], "i18next")
        self.assertEqual(payload["recommended_strategy"]["mode"], "extend-existing")
