...
```
`server.py` 起在 127.0.0.1 + 随机端口 + 随机 token。🟢 项给「移到废纸篓」(可逆) +「直接删除」(立即释放、不可逆)；🟡 项给「在访达打开」+（有安全子路径时）「移到废纸篓」。**安全模型——三套白名单，权限从严到宽**：`rm` 只允许绿灯 `trash_paths`；`trash` 允许绿灯+橙灯 `trash_paths`（橙灯永远不能 rm）；`open`（在文件管理器打开，非破坏性）允许上述全部 + 橙灯真实 `path`。每条请求：realpath+normcase 后必须命中白名单；删除(rm/trash)限 $HOME 内，`open` 另放行应用安装目录（macOS `/Applications`，Windows `Program Files`）；token 用 constant-time 比对；校验 Host + Origin 挡 CSRF/DNS-rebinding，且服务绝不回任何 `Access-Control-Allow-*`；注入报告数据前转义 `</script>` 防磁盘上恶意文件名造成存储型 XSS；每次点击浏览器先 confirm。osascript/SHFileOperationW 入废纸篓，macOS 首次弹访达自动化授权点允许即可。
...
- **Windows**：把示例里的 `python3` 换成 `python`（或 `py -3`），把 `/tmp/xxx` 换成 `%TEMP%\xxx`（Windows 没有 `/tmp`，直接照抄会报错）。若未装 Python 需先装 Python 3。建议设 `PYTHONUTF8=1` 避免 GBK 控制台乱码。
- 本 skill 是 **agent 驱动**：扫描出数据后由 agent（Claude）做分级分析，不是双击即用的独立 App。

## 平台状态

- **macOS**：完整实现并实测（扫描 / 报告 / 一键删除全验证过）。
- **Windows**：核心链路已在真实 Windows 11 + Python 3.14 实测通过——只读扫描（`scan_windows`）、token/Origin/Host/白名单/$HOME 守卫、`rm` 直删、`SHFileOperationW` 回收站删除（已修 `pFrom` 双 `\0` 截断）。多盘符已支持（主盘分段条 + 其他盘列表）。首跑仍建议核对实际目标目录是否齐全、大盘扫描耗时（实测整机约 2-3 分钟）。
