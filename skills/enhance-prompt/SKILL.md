---
name: enhance-prompt
description: Use whenever transforming vague UI generation requests into structured, high-signal prompts, optimizing prompts for Stitch or similar UI generators, or adding design system consistency to rough ideas. Make sure to use this skill for prompt engineering tasks, vague request refinement, structuring UI concepts into actionable prompts, or improving prompts that produced poor results. Also triggers for design system prompt integration, page structure specification, color token formatting, or any request involving making UI generation prompts more concrete and effective.
---
...
Before enhancing, scan the workspace to understand the project context:

1. Detect existing design documentation:
   - Look for `DESIGN.md`, `.stitch/DESIGN.md`, or `design-system.md`.
...
2. Detect Stitch project context:
   ...
   - Check for existing prompt files (`next-prompt.md`, `prompts/`).
3. Detect platform and page type:
   - Check `package.json` for web frameworks (Next.js, React) to infer web platform.
   - Look for mobile-specific configs to infer mobile platform.
   - Default to web desktop-first when no signal is found.
