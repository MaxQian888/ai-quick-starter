...
name: stitch-loop
description: |
  Use whenever you need iterative Stitch website generation with baton-style prompt handoff between autonomous build loops. Make sure to use this skill whenever the user says "build a website", "generate pages", "Stitch loop", "autonomous build", "baton", "next-prompt", or "iterative site building" — even for small sites with just a few pages. Also trigger when the user wants to set up a self-running website generation workflow, create a multi-page site from a design system, or integrate Stitch-generated pages into an existing site structure. Covers static site generation, navigation wiring, design system consistency, and loop orchestration.
---
...
Before starting the loop, detect the project state:

1. **Stitch project**: Check `.stitch/metadata.json` for project ID and existing screens.
2. **Design system**: Verify `.stitch/DESIGN.md` exists and is up-to-date.
3. **Site plan**: Review `.stitch/SITE.md` for sitemap, roadmap, and creative freedom sections.
4. **Baton state**: Check `.stitch/next-prompt.md` for the current task.
5. **Existing pages**: Look in `site/public/` for already-built pages.

Use these signals to avoid recreating existing pages and maintain design consistency.
