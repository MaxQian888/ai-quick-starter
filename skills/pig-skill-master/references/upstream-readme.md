...
在支持 Codex / AgentSkills 风格调用的环境中输入：
...
完成后用 `$pig-{slug}` 调用该群友 Skill，用 `$pig-{slug}-persona` 调用 persona-only 版本。

### 管理命令

| 命令 | 说明 |
|------|------|
| `python scripts/skill_writer.py --action list --base-dir ./pigs` | 列出所有群友 Skill |
| `python scripts/group_batch_builder.py --file <group.json> --mode dry-run --base-dir ./pigs` | 预览整份群聊里适合批量建档的人 |
| `$pig-{slug}` | 调用完整 Skill |
| `$pig-{slug}-persona` | 仅人物性格 |
| `python scripts/version_manager.py --action rollback --slug {slug} --version {version} --base-dir ./pigs` | 回滚到历史版本 |

### 批量分析整份群聊

如果你手里是整份群聊导出，而不是只想抽某一个人，现在可以先做候选预览，再批量创建：

```bash
python scripts/group_batch_builder.py \
  --file D:/Project/discovery/group_1075121298_20260408_002321.json \
  --base-dir ./pigs \
  --min-messages 20 \
  --mode dry-run
```
...
- 导出里只有数字 QQ 号，想手工映射成昵称
- 想补充某个人的职业、兴趣、标签
- 想显式 include / exclude 某些成员
...
python scripts/group_batch_builder.py \
  --file D:/Project/discovery/group_1075121298_20260408_002321.json \
  --mapping batch-mapping.json \
  --mode dry-run
```

#### 已经生成过数字 slug，后面想改名怎么办？

直接先改 `batch-mapping.json` 里的：
