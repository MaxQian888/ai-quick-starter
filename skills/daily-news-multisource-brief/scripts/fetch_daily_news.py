#!/usr/bin/env python3
"""Fetch and normalize daily news from domestic and global feeds."""
...
DEFAULT_LOOKBACK_HOURS = 30.0
USER_AGENT = "Mozilla/5.0 (compatible; daily-news-multisource-brief/1.0)"
IGNORED_QUERY_KEYS = {
    "utm_source",
    "utm_medium",
    "utm_campaign",
...
def load_sources(path: Path) -> tuple[list[dict[str, Any]], dict[str, list[str]]]:
    ...
    for source in raw["sources"]:
        ...
        category = str(source.get("category", "general")).strip().lower() or "general"
        country = str(source.get("country", "int")).strip().lower() or "int"
        language = str(source.get("language", "en")).strip().lower() or "en"
        tier = str(source.get("tier", "extended")).strip().lower() or "extended"
    ...
    if isinstance(raw_presets, dict):
        for preset_name, preset_values in raw_presets.items():
            if not isinstance(preset_values, list):
                continue
            key = str(preset_name).strip().lower()
            if not key:
                continue
            ids = [str(value).strip() for value in preset_values if str(value).strip()]
            if ids:
                presets[key] = ids
...
def parse_csv_tokens(raw_value: str) -> set[str]:
...
def filter_item(
    ...
    if categories and str(item.get("category", "")).lower() not in categories:
        return False
    if countries and str(item.get("country", "")).lower() not in countries:
        return False
    if languages and str(item.get("language", "")).lower() not in languages:
        return False
    if tiers and str(item.get("tier", "")).lower() not in tiers:
        return False
...
def to_markdown(payload: dict[str, Any]) -> str:
    lines: list[str] = []
    lines.append(f"# Daily News Snapshot ({payload['generated_at_utc']})")
    lines.append(
        "- Sources queried: "
    ...
    lines.append(
        "- Filters: "
        f"preset={','.join(payload['request']['presets']) or 'none'}, "
        f"region={payload['request']['region']}, "
        f"category={','.join(payload['request']['categories']) or 'all'}, "
        f"country={','.join(payload['request']['countries']) or 'all'}, "
        f"language={','.join(payload['request']['languages']) or 'all'}, "
        f"tier={','.join(payload['request']['tiers']) or 'all'}, "
        f"hours={payload['request']['hours']}"
    )

    if payload["failed_sources"]:
        lines.append("- Failed sources (first 15):")
        for failure in payload["failed_sources"][:15]:
            lines.append(f"  - {failure['source_name']}: {failure['error']}")
        if len(payload["failed_sources"]) > 15:
            lines.append(f"  - ... and {len(payload['failed_sources']) - 15} more")
    ...
    for idx, item in enumerate(payload["items"], start=1):
        ...
        country = item.get("country") or "int"
        language = item.get("language") or "na"
        tier = item.get("tier") or "extended"

        if link:
            lines.append(f"{idx}. [{title}]({link})")
        else:
            lines.append(f"{idx}. {title}")
        lines.append(f"   Source: {source} ({region}/{category}/{country}/{language}/{tier})")
...
def parse_extra_sources(raw_entries: list[str]) -> list[dict[str, Any]]:
    ...
    for index, raw in enumerate(raw_entries, start=1):
        ...
        if len(parts) == 1:
            ...
            name = parts[0] or name
            region = parts[1].lower() if parts[1] else region
            category = parts[2].lower() if parts[2] else category
            url = parts[3]
        elif len(parts) == 5:
            name = parts[0] or name
            region = parts[1].lower() if parts[1] else region
            category = parts[2].lower() if parts[2] else category
            country = parts[3].lower() if parts[3] else country
            url = parts[4]
        elif len(parts) == 6:
            name = parts[0] or name
            region = parts[1].lower() if parts[1] else region
            category = parts[2].lower() if parts[2] else category
            country = parts[3].lower() if parts[3] else country
            language = parts[4].lower() if parts[4] else language
            url = parts[5]
        else:
            name = parts[0] or name
            region = parts[1].lower() if parts[1] else region
            ...
            country = parts[3].lower() if parts[3] else country
            language = parts[4].lower() if parts[4] else language
            tier = parts[5].lower() if parts[5] else tier
            url = "|".join(parts[6:]).strip()
        ...
        parsed.append(
            {
                ...
                "country": country,
                "language": language,
                "tier": tier if tier in {"core", "extended", "community"} else "extended",
                "urls": [url],
            }
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Fetch daily news from multiple domestic and global sources",
    )
    parser.add_argument(
        "--sources-file",
        type=Path,
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Preset name from sources.json presets map (repeatable)",
    )
    parser.add_argument(
        "--source",
        action="append",
        default=[],
        help="Extra source using 'URL' or 'NAME|URL' or 'NAME|REGION|CATEGORY|COUNTRY|LANGUAGE|TIER|URL'",
    )
    parser.add_argument(
        "--region",
    ...
    parser.add_argument(
        "--category",
        default="",
        help="Comma-separated categories, for example politics,business,technology",
    )
    parser.add_argument(
        "--country",
        default="",
        help="Comma-separated country codes, for example cn,us,uk,jp",
    )
    parser.add_argument(
        "--language",
        default="",
        help="Comma-separated language codes, for example zh,en",
    )
    parser.add_argument(
        "--tier",
        default="",
        help="Comma-separated source tiers: core,extended,community",
    )
    parser.add_argument(
        "--keywords",
        default="",
...
def main() -> int:
    ...
    try:
        sources, presets = load_sources(args.sources_file)
    except Exception as exc:  # noqa: BLE001
    ...
    preset_names = [value.strip().lower() for value in args.preset if value.strip()]
    if preset_names:
        selected_ids: set[str] = set()
        for preset_name in preset_names:
            selected_ids.update(presets.get(preset_name, []))
        sources = [source for source in sources if source["id"] in selected_ids]

    categories = parse_csv_tokens(args.category)
    countries = parse_csv_tokens(args.country)
    languages = parse_csv_tokens(args.language)
    tiers = parse_csv_tokens(args.tier)

    if args.region != "all":
        sources = [source for source in sources if source["region"] == args.region]
    if categories:
        sources = [source for source in sources if source["category"] in categories]
    if countries:
        sources = [source for source in sources if source["country"] in countries]
    if languages:
        sources = [source for source in sources if source["language"] in languages]
    if tiers:
        sources = [source for source in sources if source["tier"] in tiers]
