...
name: ink-cli
description: |
  Use this skill whenever you are building, scaffolding, or debugging terminal CLI apps with Ink and React components.
  Make sure to use this skill whenever the user mentions Ink, terminal UI, interactive CLI, command-line interface,
  React in terminal, TUI, or asks about Box, Text, Static, useInput, useApp, useFocus, or Ink layout issues.
  Also trigger for requests like "create a CLI app", "build a terminal dashboard", "interactive terminal menu",
  "keyboard navigation in CLI", "Ink component broken", or "debug my Ink render". Covers Ink v4+ with Next.js-style
  patterns, ESM, TypeScript, and common community components like ink-spinner.
---
...
Before writing or changing Ink code, detect the project baseline:

1. **Ink version**: check `package.json` for `ink` and `react` versions to determine if migration guidance is needed.
2. **Module system**: confirm ESM (`"type": "module"`) vs CommonJS to align imports.
3. **TypeScript**: check for `tsconfig.json` and type definitions.
4. **Existing components**: look for current Ink usage patterns before introducing new ones.
5. **Test setup**: check for `ink-testing-library` or similar test utilities.
6. **Platform**: note if the CLI must run in CI (non-interactive) as well as locally.
...
**Scaffold a minimal Ink CLI with keyboard navigation:**
```bash
npx create-ink-app --typescript my-cli
cd my-cli
# Add useInput for 'q' to quit and arrow keys for navigation
```
