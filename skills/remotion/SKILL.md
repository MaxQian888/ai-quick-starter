...
name: remotion
description: >
  Use this skill whenever you need to create walkthrough videos, demo recordings, or screen-based presentations from Stitch designs using Remotion.
  Make sure to use it when the user wants to showcase app screens, build product demos, generate marketing walkthroughs, or produce programmatic video content with transitions, zoom effects, and text overlays.
  Covers screen retrieval from Stitch projects, Remotion composition setup, transition and animation design, and video rendering for React-based video production.
---

# Stitch to Remotion Walkthrough Videos

Create engaging walkthrough videos from app designs by combining Stitch screen retrieval with Remotion programmatic video generation.

## Adaptive Detection

Before starting, scan for:
- Stitch MCP Server availability and project access
- Remotion MCP Server or Remotion CLI availability
- Node.js and npm installation status
- Existing Remotion project (`remotion.config.ts` or Remotion deps in `package.json`)
- Target platform (YouTube, social media, internal demo) for dimension and format choices
...
4. **Asset download**:
   - Use `web_fetch` or local scripts to download screenshots:
     - `python scripts/download-stitch-asset.py "<downloadUrl>" "assets/screens/<screen>.png"`
     - `bash scripts/download-stitch-asset.sh "<downloadUrl>" "assets/screens/<screen>.png"`
     - `./scripts/download-stitch-asset.ps1 "<downloadUrl>" "assets/screens/<screen>.png"`
...
- `references/reading-playbook.md` — Decision rules for screen retrieval and composition strategies.
- `references/environment-matrix.md` — Platform-specific behavior and dependency strategy.
- `references/component-patterns.md` — Common Remotion composition patterns and transition recipes.

## Examples

**Example 1: Build a simple walkthrough video**
```
User: "Look up the screens in my Stitch project 'Calculator App' and build a walkthrough video."
Agent: List Stitch projects, find "Calculator App", list screens, download screenshots, create screens.json manifest, generate ScreenSlide.tsx and WalkthroughComposition.tsx, preview in Remotion Studio, and render to calculator-walkthrough.mp4.
```

**Example 2: Add a feature highlight with zoom**
```
User: "Can you highlight the new settings button in the walkthrough?"
Agent: Add an animated Hotspot component to the composition, use spring() for smooth zoom, position it over the settings button coordinates, and preview the timing before rendering.
```
