...
name: stitch-design
description: |
  Use whenever you need a unified Stitch design workflow for prompt enhancement, design-system synthesis, and screen generation. Make sure to use this skill whenever the user mentions "Stitch", "design system", "UI design", "screen design", "generate screen", "design a page", "prompt enhancement", "DESIGN.md", or "Stitch MCP" — even for minor adjustments to existing screens or extracting design tokens. Also trigger when the user needs to create consistent UI across multiple pages, refine vague design requests into professional specifications, or manage design assets downloaded from Stitch. Covers text-to-design, edit-design, and generate-design-md workflows.
---
...
Before designing, detect the project context:

1. **Stitch project**: Check if `.stitch/metadata.json` exists with a project ID.
2. **Design system**: Look for `.stitch/DESIGN.md` with existing tokens.
3. **Site plan**: Check `.stitch/SITE.md` for sitemap and roadmap context.
4. **Device type**: Determine if the design is for desktop, mobile, or tablet.
5. **Existing screens**: Review `.stitch/designs/` for already-downloaded assets.

Use these signals to maintain consistency and avoid recreating existing work.
...
### Example 1: Enhanced prompt structure

```markdown
A modern SaaS dashboard landing page with a calm, trustworthy vibe.

**DESIGN SYSTEM (REQUIRED):**
- Platform: Web, Desktop-first
- Palette: Deep Navy (#1a1a2e for primary), Soft Blue (#4a90e2 for accent)
- Styles: Medium roundness (8px), subtle shadows

**PAGE STRUCTURE:**
1. **Header:** Sticky nav with logo left, CTA right
2. **Hero Section:** Headline, subtext, and primary button
3. **Features Grid:** 3 cards with icons
...
- [Prompting Keywords](references/prompting-keywords.md) — Technical terms Stitch understands best.
