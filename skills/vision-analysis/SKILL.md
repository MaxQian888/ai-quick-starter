...
Before analyzing, detect the image context:

1. **Image source**: Extract file path or URL from the user message.
2. **Analysis goal**: Determine if the user wants description, OCR, UI review, chart data, or object detection.
3. **MCP availability**: Check if `MiniMax_understand_image` is available via `list_tools`.
4. **Image format**: Confirm the file is a supported format (JPEG, PNG, GIF, WebP, BMP, SVG).
5. **Size constraints**: Note that images up to 20MB are supported.

Use these signals to choose the correct mode and prompt strategy.
...
### Example 1: Describe a screenshot

```
User: "What do you see in this screenshot?"
Mode: describe
Tool: MiniMax_understand_image with describe prompt
```

### Example 2: Extract text from a document

```
User: "Read all the text from this document image"
Mode: ocr
Tool: MiniMax_understand_image with ocr prompt
```
