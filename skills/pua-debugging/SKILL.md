---
name: pua-debugging
description: "Use when a task fails 2+ times or the agent becomes passive: giving up, deflecting to the user, blaming the environment without proof, repeating ineffective tweaks, fixing only surface issues, skipping verification, suggesting instead of executing, or stopping after permission/network/auth errors without trying alternatives. Also trigger on user frustration phrases (e.g., '你怎么又失败了', '为什么还不行', '换个方法', '你再试试', '不要放弃', '继续', '加油'). Applies to all task types. Do not trigger on first-attempt failures or when a known fix is already in progress."
---
